...
const paths = {
    ...
    jsBundle: {
        src: ['../sys/build/sys/js/**/*'],
        dest: './build/sys/js',
        watch: '../sys/build/sys/js/**/*',
    },
    images: {
        src: './src/blocks/**/img/*',
        dest: './build/img',
...
gulp.task('styles', function () {
    return gulp
        .src(paths.css.src, { sourcemaps: false })
        ...
        .pipe(gulp.dest(paths.css.dest, { sourcemaps: false }))
        .on('error', function () {
            this.emit('end');
        })
        .pipe(browserSync.reload({ stream: true }));
...
gulp.task('backgrounds', function () {
    return gulp
        ...
        .pipe(plumber())
        .pipe(postcss([
            bgImage({ mode: 'cutterInvertor' }),
            webpInCss(),
            cssnano(),
        ]))
        .pipe(rename({ suffix: '2.bg' }))
        .pipe(gulp.dest(paths.css.dest));
});

gulp.task('add backgrounds', function () {
    return gulp
        .src('./build/css/*')
...
gulp.task('add local bundle.js', function () {
    return gulp.src(paths.jsBundle.src).pipe(gulp.dest(paths.jsBundle.dest));
});
...
gulp.task('server', function () {
    ...
    gulp.watch(paths.css.watch, gulp.series('styles', 'backgrounds', 'add backgrounds', 'clean style'));
...
// gulp.task('dev', gulp.series('build with bundle.js', 'server'));
